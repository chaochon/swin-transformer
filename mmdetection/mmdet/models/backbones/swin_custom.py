...
class SwinBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.low_freq_size = nn.Parameter(torch.tensor(0.25, dtype=torch.float32))  # 初始化为 0.5
    ...
    def forward(self, x, hw_shape):
        ...
        def _inner_forward(x):
            ...
            # 使用平滑可求导的方式，计算 low_freq_size
            cutoff_ratio = torch.sigmoid(self.low_freq_size) * 0.5
            high_freq_x = extract_high_freq_1d_feat(x, cutoff_ratio)

            x = self.attn(x, hw_shape)

            x = x + identity + high_freq_x
...
class SwinTransformerCustom(BaseModule):
    ...
    def __init__(self,
        ...
        super(SwinTransformerCustom, self).__init__(init_cfg=init_cfg)
...
def extract_high_freq_feat(x, low_freq_size=8):
        ...
        high_freq_x = img_reconstructed.real.mean(dim=1, keepdim=True)
...
def extract_high_freq_1d_feat(x, cutoff_ratio):
    '''
    Args:
        x: shape(batch, token_num, embeding)
        cutoff_ratio:  [0.0-0.5]
    Returns:
        res: high-pass filter freq
    '''
    # 获取输入形状
    batch, token_num, embedding = x.shape

    if cutoff_ratio > 0.5:
        logger = MMLogger.get_current_instance()
        logger.warn('Warning: cutoff_ratio:{} is greater than 0.5 and it meas mask of high-pass filter is full zero'.format(cutoff_ratio))
        cutoff_ratio = 0.5
    ...
    # 构造高通滤波器
    cutoff = int(cutoff_ratio * token_num)  # 截止频率索引
    mask = torch.ones_like(freq, dtype=torch.complex64)  # 初始化掩码为全1
    mask[:, :cutoff, :] = 0  # 屏蔽低频成分
    mask[:, -cutoff:, :] = 0  # 屏蔽高频对称部分

    # 应用高通滤波
    high_pass_freq = freq * mask

    # 傅里叶逆变换
    res = torch.fft.ifft(high_pass_freq, dim=1).real  # 取实部
    return res

def test_extract_high_freq_1d_feat():
    inputs = torch.randn(2, 40000, 96)
    out = extract_high_freq_1d_feat(inputs, 0.5)
    print(out.shape)
...
if __name__ == '__main__':
    ...
    #test_extract_high_freq_1d_feat()
